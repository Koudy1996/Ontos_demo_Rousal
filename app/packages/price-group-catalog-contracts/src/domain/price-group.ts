...
export const PriceGroupSameMeaningDecisionSchema = Schema.Struct({
  comparedDefinitionRevisionId: PriceGroupDefinitionRevisionIdSchema,
  decision: Schema.Literal('SAME_MEANING'),
  provenance: PriceGroupAuditProvenanceSchema,
});
export type PriceGroupSameMeaningDecision = typeof PriceGroupSameMeaningDecisionSchema.Type;
...
export const PriceGroupDefinitionRevisionSchema = Schema.Struct({
  ...
  // oxlint-disable-next-line effect-native/no-nullable-schema-field -- null identifies the initial revision; every successor carries its owner-issued SAME_MEANING decision; expires: 2027-03-31.
  semanticContinuity: Schema.NullOr(PriceGroupSameMeaningDecisionSchema),
}).check(
  Schema.makeFilter((revision) => {
    ...
    const continuityContractIsValid =
      (revision.revisionNumber === 1 && revision.semanticContinuity === null) ||
      (revision.revisionNumber > 1 &&
        revision.previousDefinitionRevisionId !== null &&
        revision.previousDefinitionRevisionId !== revision.definitionRevisionId &&
        revision.semanticContinuity !== null &&
        revision.semanticContinuity.comparedDefinitionRevisionId === revision.previousDefinitionRevisionId);
    if (!continuityContractIsValid) {
      return 'Every successor Price Group definition must carry a SAME_MEANING decision for its exact predecessor';
    }
...
const samePriceGroupRef = (left: StablePriceGroupRef, right: StablePriceGroupRef): boolean =>
  left.moduleId === right.moduleId &&
  left.resourceId === right.resourceId &&
  left.resourceType === right.resourceType &&
  left.tenantId === right.tenantId;

const sameAuditProvenance = (left: PriceGroupAuditProvenance, right: PriceGroupAuditProvenance): boolean =>
  left.actionInvocationId === right.actionInvocationId &&
  left.actorPrincipalId === right.actorPrincipalId &&
  left.reason === right.reason &&
  left.trustedAt === right.trustedAt;

export const PriceGroupDefinitionAcceptanceSchema = Schema.Struct({
  acceptedCatalogRevision: CatalogRevisionSchema,
  definitionEffectivePeriod: PriceGroupEffectivePeriodSchema,
  definitionRevisionId: PriceGroupDefinitionRevisionIdSchema,
  definitionRevisionNumber: PriceGroupDefinitionRevisionNumberSchema,
  ...
  Schema.makeFilter((acceptance) =>
    acceptance.provenance.trustedAt <= acceptance.definitionEffectivePeriod.effectiveFrom
      ? undefined
      : 'Definition acceptance cannot be recorded after the accepted effective start',
  ),
);
export type PriceGroupDefinitionAcceptance = typeof PriceGroupDefinitionAcceptanceSchema.Type;

export const PriceGroupCreatedResultSchema = Schema.Struct({
  acceptance: PriceGroupDefinitionAcceptanceSchema,
  identity: PriceGroupIdentitySchema,
  initialDefinition: PriceGroupDefinitionRevisionSchema,
}).check(
  Schema.makeFilter((result) => {
    const { acceptance, identity, initialDefinition } = result;
    const exactInitialDefinition =
      initialDefinition.revisionNumber === 1 &&
      initialDefinition.previousDefinitionRevisionId === null &&
      initialDefinition.semanticContinuity === null;
    const evidenceMatchesDefinition =
      acceptance.definitionRevisionId === initialDefinition.definitionRevisionId &&
      acceptance.definitionRevisionNumber === initialDefinition.revisionNumber &&
      acceptance.meaningFingerprint === initialDefinition.meaningFingerprint &&
      acceptance.acceptedCatalogRevision === initialDefinition.acceptedCatalogRevision &&
      acceptance.definitionEffectivePeriod.effectiveFrom === initialDefinition.effectivePeriod.effectiveFrom &&
      acceptance.definitionEffectivePeriod.effectiveTo === initialDefinition.effectivePeriod.effectiveTo &&
      sameAuditProvenance(acceptance.provenance, initialDefinition.created);
    const identityMatchesDefinition =
      identity.meaningFingerprint === initialDefinition.meaningFingerprint &&
      identity.createdAtCatalogRevision === acceptance.acceptedCatalogRevision &&
      sameAuditProvenance(identity.created, acceptance.provenance) &&
      samePriceGroupRef(identity.priceGroupRef, initialDefinition.priceGroupRef) &&
      samePriceGroupRef(acceptance.priceGroupRef, initialDefinition.priceGroupRef) &&
      identity.lifecycle.state === 'ACTIVE' &&
      identity.lifecycle.retiredAt === null &&
      identity.lifecycle.activeFrom === initialDefinition.effectivePeriod.effectiveFrom;
    return exactInitialDefinition && evidenceMatchesDefinition && identityMatchesDefinition
      ? undefined
      : 'Create Price Group result must bind one stable identity to its exact initial accepted definition';
  }),
);
export type PriceGroupCreatedResult = typeof PriceGroupCreatedResultSchema.Type;
...
export const PriceGroupRetirementEvidenceSchema = Schema.Struct({
  ...
  Schema.makeFilter((evidence) => {
    if (evidence.retirementProvenance.trustedAt > evidence.retiredAt) {
      return 'Retirement provenance cannot be recorded after the effective retirement boundary';
    }
    return evidence.retiredAt <= evidence.trustedOperationAt && evidence.trustedOperationAt <= evidence.verifiedAt
      ? undefined
