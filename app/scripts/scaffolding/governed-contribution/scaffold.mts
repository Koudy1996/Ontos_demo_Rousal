...
import { planActionBoundaryScaffold, renderActionGatewayClient } from '../microvertical-action-boundary/scaffold.mts';
import {
...
const PublicContractPackageSchema = Schema.Struct({
  dependencies: Schema.Record(Schema.String, Schema.String),
  exports: Schema.Record(Schema.String, Schema.String),
  name: Schema.String,
  ontosContractOwner: Schema.Struct({
    appId: ContractOwnerAppIdSchema,
    moduleApis: Schema.Array(Schema.String),
    moduleId: ContractOwnerModuleIdSchema,
    packageName: Schema.String,
  }),
});

interface PublicContractPackage {
...
const isGovernedHttpHandlerLayerSlotEntry = (candidate: string): boolean => {
  const isRead =
    /^[A-Za-z][A-Za-z0-9]*ReadApiLive\.pipe\(/u.test(candidate) &&
    candidate.includes('GovernedReadLayer.provide(governedReadRuntimeLive)');
  const isAction =
    /^[A-Za-z][A-Za-z0-9]*ActionApiLive\.pipe\(/u.test(candidate) &&
    candidate.includes('GovernedReadLayer.provide(governedActionRuntimeLive)');
  return (isRead || isAction) && candidate.endsWith('),');
};
  ...
  function* discoverPublicContractPackage(workspaceRoot: string, vertical: OntosVerticalMetadata, name: string) {
    ...
    for (const entry of entries.toSorted()) {
      ...
      const source = yield* fileSystem
        ...
        .pipe(
          Effect.mapError((cause) => scaffoldFailure(`failed to read public contract package ${packagePath}`, cause)),
        );
...
const renderApiClient = (
  vertical: OntosVerticalMetadata,
  name: string,
  contractImport = `../../shared/apis/${name}.ts`,
): string => {
  const type = toPascalCase(name);
  const value = `${toPascalCase(name)}Api`;
...
import { ${value} } from '${contractImport}';
import type { ${type}Request } from '${contractImport}';
  ...
  function* patchGovernedHttpComposition(
    ...
    const nextSharedApi = yield* tryScaffold('failed to patch governed HTTP API root', () =>
      insertSortedSlotIdempotently(
        insertSortedSlotIdempotently(
          stabilizeGovernedHttpApiAdditionSlot(sharedApi).replace(
    ...
    const nextHandlerRoot = yield* tryScaffold('failed to patch governed HTTP handler root', () => {
      let next = insertSortedSlotIdempotently(
        ...
        isGovernedHttpHandlerLayerSlotEntry,
      );
...
const acceptsGeneratedClient = (
  ...
  vertical: OntosVerticalMetadata,
  name: string,
  moduleApiContractImport = `../../shared/apis/${name}.ts`,
...
const planGovernedTransport = Effect.fn('GovernedContributionScaffold.transport')(function* planGovernedTransport(
  ...
  if (kind !== PUBLIC_COMPONENT_KIND) {
    ...
    if (isApi && publicContractPackage !== undefined) {
      const canonicalClientMutation = yield* createOrAcceptGeneratedMutationEffect(
        publicContractPackage.clientPath,
        renderApiClient(vertical, name, `../apis/${name}.ts`),
        acceptsGeneratedClient(kind, vertical, name, `../apis/${name}.ts`),
      );
      const ownerClientMutation = yield* createOrAcceptGeneratedMutationEffect(
        clientPath,
        renderPublicContractShim(publicContractPackage.clientSpecifier),
        (current) => current === renderPublicContractShim(publicContractPackage.clientSpecifier),
      );
      const packageGatewayPath = yield* tryScaffold('failed to resolve public contract operation gateway path', () =>
        resolveContainedPath(publicContractPackage.directory, 'src', 'api', 'action-gateway.ts'),
      );
      const packageGatewayMutation = yield* createOrAcceptGeneratedMutationEffect(
        packageGatewayPath,
        renderActionGatewayClient(vertical),
        (current) => hasGeneratedOperationGatewayContract(current, vertical.appId),
      );
      mutations.push(...EffectArray.getSomes([canonicalClientMutation, ownerClientMutation, packageGatewayMutation]));
    } else {
...
export const planGovernedContributionScaffold = Effect.fn('GovernedContributionScaffold.plan')(
  function* planGovernedContributionScaffold(
    ...
    const isApi = kind === MODULE_API_KIND;
    const publicContractPackage = isApi
      ? yield* discoverPublicContractPackage(workspaceRoot, vertical, name)
      : undefined;
