...
const ACTION_BOUNDARY_GENERATOR_HEADER = '// @generated by OntOS Codesmith MicroVertical Action Boundary v1';
...
> =>
  Effect.gen(function* planActionBoundaryScaffoldEffect() {
    ...
    const clientPath = yield* trySync(() =>
      resolveContainedPath(workspaceRoot, 'verticals', vertical.slug, 'src', 'api', 'action-gateway.ts'),
    );
    const redemptionPath = yield* trySync(() =>
      resolveContainedPath(workspaceRoot, 'verticals', vertical.slug, 'api', 'auth', 'gateway-assertion-redemption.ts'),
    );
    const runnerPath = yield* trySync(() =>
      resolveContainedPath(workspaceRoot, 'verticals', vertical.slug, 'api', 'action-http-runner.ts'),
    );
    ...
    const runnerMutation = yield* createOrAcceptOwnedMutation(runnerPath, renderActionHttpRunner(vertical), [
      'bindActionHttpRunner',
      `@ontos-action-boundary-owner ${vertical.appId}`,
    ]);
