  ...
  Effect.fn(function* scenario65() {
    yield* withFixture(
      Effect.fn(function* scenario66(fixture) {
        ...
        expect(message).toBe(`// @generated by OntOS Codesmith Outbox Message v2
// @ontos-outbox-action inventory.stock.create-order
// @ontos-outbox-topic orders.created
import type { OutboxMessage } from '@app/core-runtime';
import { outboxProducerModuleKey, outboxTopic } from '@app/inventory-stock/outbox/orders-created';
import type { OutboxPayload } from '@app/inventory-stock/outbox/orders-created';

export {
  OutboxPayloadSchema as CreateOrderOrdersCreatedOutboxPayloadSchema,
  outboxProducerModuleKey as CreateOrderOrdersCreatedOutboxProducerModuleKey,
  outboxTopic as CreateOrderOrdersCreatedOutboxTopic,
} from '@app/inventory-stock/outbox/orders-created';
export type { OutboxPayload as CreateOrderOrdersCreatedOutboxPayload } from '@app/inventory-stock/outbox/orders-created';
...
`);
        ...
        const canonicalShippedMessage =
          'verticals/inventory-stock/src/actions/create-order-orders-shipped.outbox-message.ts';
        const legacyShippedMessage =
          'verticals/inventory-stock/src/actions/create-order.orders-shipped.outbox-message.ts';
        yield* Effect.promise(() =>
          rename(path.join(fixture.root, canonicalShippedMessage), path.join(fixture.root, legacyShippedMessage)),
        );
        const legacyShippedPath = path.join(fixture.root, legacyShippedMessage);
        const legacyShippedContent = (yield* Effect.promise(() => readFile(legacyShippedPath, 'utf-8'))).replace(
          `// @generated by OntOS Codesmith Outbox Message v2
// @ontos-outbox-action inventory.stock.create-order
// @ontos-outbox-topic orders.shipped
`,
          '',
        );
        yield* Effect.promise(() => writeFile(legacyShippedPath, legacyShippedContent, 'utf-8'));
        const actionWithLegacyMessagePath = (yield* Effect.promise(() => readFile(actionPath, 'utf-8'))).replaceAll(
          './create-order-orders-shipped.outbox-message.ts',
          './create-order.orders-shipped.outbox-message.ts',
        );
        yield* Effect.promise(() => writeFile(actionPath, actionWithLegacyMessagePath, 'utf-8'));
        yield* run(fixture, scaffoldCommand.outboxMessage, [
          scaffoldFlag.vertical,
          inventorySlug,
          '--action',
        ...
        const migratedTree = yield* snapshotTree(fixture.root);
        expect(migratedTree[canonicalShippedMessage]).toBeDefined();
        expect(migratedTree[legacyShippedMessage]).toBeUndefined();
  ...
  Effect.fn(function* scenario67() {
    yield* withFixture(
      Effect.fn(function* scenario68(fixture) {
        ...
        // Re-running the same generated topic is intentionally idempotent.
        yield* run(fixture, scaffoldCommand.outboxMessage, [
          scaffoldFlag.vertical,
          inventorySlug,
          '--action',
        ...
        // A topic that normalizes to the same path but has a different identity
        // must still be rejected without mutating the fixture.
        yield* expectFailure(
