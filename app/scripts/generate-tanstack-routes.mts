...
const generateRouteMetadataManifest = (appDirectory: string, appId: string, moduleId: string, workspaceRoot: string) =>
  Effect.gen(function* generateRouteMetadataManifestEffect() {
    ...
    // Keep repeated owner/path values named in generated TypeScript without changing metadata semantics.
    const commonValues = [
      ...new Set([
        namespace,
        moduleId,
        ...routes.flatMap((route) => [route.canonicalPath, ...Object.values(route.localisedPaths)]),
      ]),
    ];
    const literals = yield* Effect.forEach(
      commonValues,
      (value, index) =>
        encodeJsonString(value).pipe(
          Effect.map((encoded) => ({
            encoded,
            name: index === 0 ? 'ultramodernRouteNamespace' : `routeValue${index}`,
          })),
          Effect.mapError(() => failure(`Unable to encode shared route metadata for ${appId}`)),
        ),
      { concurrency: 1 },
    );
    const declarations = literals
      .slice(1)
      .map(({ encoded, name }) => `const ${name} = ${encoded};`)
      .join('\n');
    const withNamedValues = (encoded: string) => {
      let result = encoded;
      for (const literal of literals) {
        result = result.replaceAll(`: ${literal.encoded}`, `: ${literal.name}`);
      }
      return result;
    };
...
export const ultramodernRouteMetadata = ${withNamedValues(encodedRoutes)} as const;

export const ultramodernLocalisedUrls = ${withNamedValues(encodedLocalisedUrls)} as const;
