...
const provisionContactsAccess = Effect.fn('provisionContactsAccess')(function* provisionContactsAccessEffect(
  e2eTenants: FixtureTenants,
) {
      ...
      Effect.gen(function* makeContactsRelationships() {
        ...
        return (
          ...
          v1.Relationship.create({
            ...
            subject: {
              object: { objectId: subjectId, objectType: subjectType },
            },
          }),
        );
      }),
  ...
  yield* Effect.addFinalizer(() => update(v1.RelationshipUpdate_Operation.DELETE).pipe(Effect.orDie));
...
export const createAuthenticationFixture = Effect.fn('createAuthenticationFixture')(
  function* createAuthenticationFixtureEffect() {
    ...
    const cleanup = Effect.fn('cleanupAuthenticationFixture')(function* cleanupAuthenticationFixtureEffect() {
      ...
      yield* coreDatabase.delete(dataAccessEvents).where(inArray(dataAccessEvents.principalId, principalIds));
      ...
      yield* coreDatabase.delete(principalAuthBindings).where(inArray(principalAuthBindings.principalId, principalIds));
      yield* coreDatabase.delete(tenantModuleStates).where(inArray(tenantModuleStates.tenantId, tenantIds));
