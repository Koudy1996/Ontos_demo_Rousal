...
const appId = 'workforce';
const bffPrefix = '/workforce-api';
const cloudflareWorkerName = 'app-workforce';
const build = createModernBuildContext({
  appId,
  cloudflarePublicUrlEnvironmentVariable: 'ULTRAMODERN_PUBLIC_URL_WORKFORCE',
  cloudflareWorkerName,
  defaultPort: 4111,
  getBuildConfigEnvironment,
  portEnvironmentVariable: 'VERTICAL_WORKFORCE_PORT',
});
const resolveApiBaseUrl = () => {
  const publicUrl = build.envValue('ULTRAMODERN_PUBLIC_URL_WORKFORCE');
  if (publicUrl !== undefined) {
    return new URL(bffPrefix, publicUrl).href;
  }
  if (build.cloudflareDeployEnabled) {
    throw new Error('Workforce deployment requires ULTRAMODERN_PUBLIC_URL_WORKFORCE.');
  }
...
const config = createModernConfig({
  ...
  bffPrefix,
  build,
  chunkLoadingGlobal: '__ULTRAMODERN_VERTICAL_WORKFORCE_LOADED_CHUNKS__',
  cloudflareWorkerName,
  ...
  presetUltramodern(
    {
      ...
      source: {
        ...
        globalVars: {
          ...
          ULTRAMODERN_WORKFORCE_API_BASE_URL: resolveApiBaseUrl(),
        },
      },
    },
    {
      appId,
      deliveryUnit: { buildMarker: '36eece9daae1ace2', unitId: 'app/workforce', version: '0.1.0' },
    },
  ),
);
