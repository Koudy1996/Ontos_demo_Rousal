...
import { gatewayAssertionRedemptions, invoiceNumberCounters, invoices } from '../src/db/schema.ts';

class BillingDocumentsDatabaseVerificationError extends Schema.TaggedError<BillingDocumentsDatabaseVerificationError>()(
  'BillingDocumentsDatabaseVerificationError',
  { reason: Schema.String },
) {}

const verification = Effect.gen(function* verifyBillingDocumentsDatabase() {
  const database = yield* makeWithDefaults({});
  const tables = [gatewayAssertionRedemptions, invoiceNumberCounters, invoices];
  for (const table of tables) {
    yield* database.select().from(table).limit(0);
  }
  const inventory = yield* database.execute<{ name: string }>(
    sql`select relname as name from pg_class join pg_namespace on pg_namespace.oid=pg_class.relnamespace where nspname='billing_documents' and relkind='r' order by relname`,
    'objects',
  );
  ...
  if (inventory.length !== expected.length || inventory.some((row, index) => row.name !== expected[index])) {
    return yield* new BillingDocumentsDatabaseVerificationError({
      reason: 'Billing Documents table inventory mismatch',
    });
  }
  const security = yield* database.execute<{
    ...
    cross join pg_roles where nspname='billing_documents' and relname in ('invoices','invoice_number_counters') and rolname='ontos_runtime' order by relname`,
    'objects',
  );
  if (
    security.length !== 2 ||
    ...
    return yield* new BillingDocumentsDatabaseVerificationError({
      reason: 'Billing Documents RLS or runtime grants mismatch',
    });
  }
  const journal = yield* database.execute<{ count: number }>(
    sql`select count(*)::integer as count from drizzle.__drizzle_migrations_billing_documents`,
    'objects',
  );
  if (journal[0]?.count !== 1) {
    return yield* new BillingDocumentsDatabaseVerificationError({
      reason: 'Billing Documents migration journal mismatch',
    });
  }
  return yield* Console.log('Verified Billing Documents tables, migration journal, forced RLS and runtime privileges');
});
