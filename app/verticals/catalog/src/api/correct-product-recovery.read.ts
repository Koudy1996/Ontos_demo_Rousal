...
import {
  CorrectProductRecoveryRequestSchema,
  CorrectProductRecoveryResponseSchema,
...
import { CorrectProductResultSchema } from '../../shared/actions/correct-product.ts';
...
const notFound = () =>
  new ReadHandlerNotFound({
    ...
    reason: 'No committed correct product result exists in this principal scope',
  });
const unavailable = () =>
  new ReadHandlerUnavailable({
    code: 'read_handler_unavailable',
    reason: 'The original correct product outcome cannot be verified yet',
  });
...
const recoverResult = (
  ...
  recoverCatalogActionResult(
    transaction,
    scope,
    { actionInvocationId: invocationId, actionKey: 'commerce.catalog.correct-product', schemaVersion: 1 },
    {
...
export const correctProductRecoveryRead = defineRead(
  {
    accessKind: 'detail',
    entrypoint: correctProductRecoveryEntrypoint,
    evidencePolicy: {
      captureMode: 'metadata_only',
      policyKey: 'commerce.catalog.api.correct-product-recovery.evidence.v1',
    },
    inputSchema: CorrectProductRecoveryRequestSchema,
    legalEntityScope: 'forbidden',
    owningModuleKey: 'commerce.catalog',
    permissionTarget: 'tenant',
    policies: [],
    readKey: 'commerce.catalog.api.correct-product-recovery',
    resultSchema: CorrectProductRecoveryResponseSchema,
    schemaVersion: '1',
  },
  (input, context: ReadHandlerContext<RecoveryService>) =>
    recoverCorrectProduct(input, context).pipe(Effect.map((result) => ({ evidence: { resultCount: 1 }, result }))),
  (transaction, scope) =>
