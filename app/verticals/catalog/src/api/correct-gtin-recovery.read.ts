...
import {
  CorrectGtinRecoveryRequestSchema,
  CorrectGtinRecoveryResponseSchema,
...
import { CorrectGtinResultSchema } from '../../shared/actions/correct-gtin.ts';
...
const notFound = () =>
  new ReadHandlerNotFound({
    ...
    reason: 'No committed correct gtin result exists in this principal scope',
  });
const unavailable = () =>
  new ReadHandlerUnavailable({
    code: 'read_handler_unavailable',
    reason: 'The original correct gtin outcome cannot be verified yet',
  });
...
const recoverResult = (
  ...
  recoverCatalogActionResult(
    transaction,
    scope,
    { actionInvocationId: invocationId, actionKey: 'commerce.catalog.correct-gtin', schemaVersion: 1 },
    {
...
export const correctGtinRecoveryRead = defineRead(
  {
    accessKind: 'detail',
    entrypoint: correctGtinRecoveryEntrypoint,
    evidencePolicy: {
      captureMode: 'metadata_only',
      policyKey: 'commerce.catalog.api.correct-gtin-recovery.evidence.v1',
    },
    inputSchema: CorrectGtinRecoveryRequestSchema,
    legalEntityScope: 'forbidden',
    owningModuleKey: 'commerce.catalog',
    permissionTarget: 'tenant',
    policies: [],
    readKey: 'commerce.catalog.api.correct-gtin-recovery',
    resultSchema: CorrectGtinRecoveryResponseSchema,
    schemaVersion: '1',
  },
  (input, context: ReadHandlerContext<RecoveryService>) =>
    recoverCorrectGtin(input, context).pipe(Effect.map((result) => ({ evidence: { resultCount: 1 }, result }))),
  (transaction, scope) =>
