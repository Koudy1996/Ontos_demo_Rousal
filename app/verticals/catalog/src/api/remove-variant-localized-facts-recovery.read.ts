...
import {
  RemoveVariantLocalizedFactsRecoveryRequestSchema,
  RemoveVariantLocalizedFactsRecoveryResponseSchema,
...
import { RemoveVariantLocalizedFactsResultSchema } from '../../shared/actions/remove-variant-localized-facts.ts';
...
const recoverResult = (
  ...
  recoverCatalogActionResult(
    ...
    {
      decode: Schema.decodeUnknownEffect(RemoveVariantLocalizedFactsResultSchema),
      encode: Schema.encodeEffect(RemoveVariantLocalizedFactsResultSchema),
    },
  );
...
export const removeVariantLocalizedFactsRecoveryEntrypoint = defineTenantModuleEntrypoint({
  ...
  entrypointKey: 'commerce.catalog.api.remove-variant-localized-facts-recovery',
  moduleKey: 'commerce.catalog',
  role: 'api',
});

export const removeVariantLocalizedFactsRecoveryRead = defineRead(
  {
    accessKind: 'detail',
    entrypoint: removeVariantLocalizedFactsRecoveryEntrypoint,
    evidencePolicy: {
      captureMode: 'metadata_only',
      policyKey: 'commerce.catalog.api.remove-variant-localized-facts-recovery.evidence.v1',
    },
    inputSchema: RemoveVariantLocalizedFactsRecoveryRequestSchema,
    legalEntityScope: 'forbidden',
    owningModuleKey: 'commerce.catalog',
    permissionTarget: 'tenant',
    policies: [],
    readKey: 'commerce.catalog.api.remove-variant-localized-facts-recovery',
    resultSchema: RemoveVariantLocalizedFactsRecoveryResponseSchema,
    schemaVersion: '1',
  },
  (input, context: ReadHandlerContext<RecoveryService>) =>
    recoverRemoveVariantLocalizedFacts(input, context).pipe(
