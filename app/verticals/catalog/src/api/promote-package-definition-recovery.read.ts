...
import {
  PromotePackageDefinitionRecoveryRequestSchema,
  PromotePackageDefinitionRecoveryResponseSchema,
...
import { PromotePackageDefinitionResultSchema } from '../../shared/actions/promote-package-definition.ts';
...
const notFound = () =>
  new ReadHandlerNotFound({
    ...
    reason: 'No committed promote package definition result exists in this principal scope',
  });
const unavailable = () =>
  new ReadHandlerUnavailable({
    code: 'read_handler_unavailable',
    reason: 'The original promote package definition outcome cannot be verified yet',
  });
...
const recoverResult = (
  ...
  recoverCatalogActionResult(
    transaction,
    scope,
    { actionInvocationId: invocationId, actionKey: 'commerce.catalog.promote-package-definition', schemaVersion: 1 },
    {
...
export const promotePackageDefinitionRecoveryEntrypoint = defineTenantModuleEntrypoint({
  ...
  entrypointKey: 'commerce.catalog.api.promote-package-definition-recovery',
  moduleKey: 'commerce.catalog',
  role: 'api',
});

export const promotePackageDefinitionRecoveryRead = defineRead(
  {
    accessKind: 'detail',
    entrypoint: promotePackageDefinitionRecoveryEntrypoint,
    evidencePolicy: {
      captureMode: 'metadata_only',
      policyKey: 'commerce.catalog.api.promote-package-definition-recovery.evidence.v1',
    },
    inputSchema: PromotePackageDefinitionRecoveryRequestSchema,
    legalEntityScope: 'forbidden',
    owningModuleKey: 'commerce.catalog',
    permissionTarget: 'tenant',
    policies: [],
    readKey: 'commerce.catalog.api.promote-package-definition-recovery',
    resultSchema: PromotePackageDefinitionRecoveryResponseSchema,
    schemaVersion: '1',
  },
  (input, context: ReadHandlerContext<RecoveryService>) =>
    recoverPromotePackageDefinition(input, context).pipe(
