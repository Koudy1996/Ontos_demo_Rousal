...
import {
  ConfirmGtinRecoveryRequestSchema,
  ConfirmGtinRecoveryResponseSchema,
...
import { ConfirmGtinResultSchema } from '../../shared/actions/confirm-gtin.ts';
...
const notFound = () =>
  new ReadHandlerNotFound({
    ...
    reason: 'No committed confirm gtin result exists in this principal scope',
  });
const unavailable = () =>
  new ReadHandlerUnavailable({
    code: 'read_handler_unavailable',
    reason: 'The original confirm gtin outcome cannot be verified yet',
  });
...
const recoverResult = (
  ...
  recoverCatalogActionResult(
    transaction,
    scope,
    { actionInvocationId: invocationId, actionKey: 'commerce.catalog.confirm-gtin', schemaVersion: 1 },
    {
...
export const confirmGtinRecoveryRead = defineRead(
  {
    accessKind: 'detail',
    entrypoint: confirmGtinRecoveryEntrypoint,
    evidencePolicy: {
      captureMode: 'metadata_only',
      policyKey: 'commerce.catalog.api.confirm-gtin-recovery.evidence.v1',
    },
    inputSchema: ConfirmGtinRecoveryRequestSchema,
    legalEntityScope: 'forbidden',
    owningModuleKey: 'commerce.catalog',
    permissionTarget: 'tenant',
    policies: [],
    readKey: 'commerce.catalog.api.confirm-gtin-recovery',
    resultSchema: ConfirmGtinRecoveryResponseSchema,
    schemaVersion: '1',
  },
  (input, context: ReadHandlerContext<RecoveryService>) =>
    recoverConfirmGtin(input, context).pipe(Effect.map((result) => ({ evidence: { resultCount: 1 }, result }))),
  (transaction, scope) =>
