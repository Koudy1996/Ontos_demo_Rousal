...
interface PriceGroupDefinitionSnapshot {
  ...
  readonly scheduledRetirement?: PriceGroupRetirementAcceptance;
}
...
interface CreatePriceGroupDefinitionRevisionInput {
  ...
  readonly reason: string;
  readonly semanticDecision:
    | { readonly comparedDefinitionRevisionId: string; readonly decision: 'SAME_MEANING' }
    | { readonly decision: 'MATERIAL_CHANGE' };
  readonly trustedEffectiveAt: Date;
}
...
export interface PriceGroupCatalogPersistence {
  ...
  ) => Effect.Effect<CreatePriceGroupPersistenceResult, PriceGroupPersistenceFailure>;
...
export interface CreatePriceGroupPersistenceResult {
  readonly businessResult: PriceGroupCreatedResult;
  readonly projection: typeof PriceGroupContainmentProjectionReconciliationSchema.Type;
}
      ...
      semantic_identity_conflict: ({ existingMeaningFingerprint, existingPriceGroupId, requestedMeaningFingerprint }) =>
        Effect.fail(
          new PriceGroupSemanticIdentityConflict({
            ...
            existingPriceGroupRef: placeholderRef(priceGroupRef.tenantId, existingPriceGroupId),
...
const mapCreatePriceGroupOutcome = (
  ...
  actionInvocationId: string,
  businessCode: string,
): Effect.Effect<CreatePriceGroupPersistenceResult, PriceGroupOwnerFailure> => {
  if (Schema.is(CreatePriceGroupMutationSuccessSchema)(outcome)) {
    const { definition } = outcome;
    const businessResult = PriceGroupCreatedResultSchema.make({
      acceptance: {
...
const mapRetirementOutcome = (
  ...
  Match.value(outcome).pipe(
    Match.tags({
      ...
      retirement_effective_time_conflict: ({ effectiveAt, reason, trustedEffectiveAt }) =>
        Effect.fail(
          new PriceGroupRetirementEffectiveTimeConflict({
            code: 'price_group_retirement_effective_time_conflict',
            effectiveAt,
            priceGroupRef,
            reason: reason ?? 'A Price Group retirement cannot become effective before its trusted operation time',
            trustedEffectiveAt,
          }),
        ),
    }),
...
export const priceGroupCatalogPersistenceFromRoutineInvoker = (
  ...
  const readCurrentDefinition: PriceGroupCatalogPersistence['readCurrentDefinition'] = Effect.fn(
    ...
    return yield* Match.value(outcome).pipe(
      Match.tags({
        current: ({ catalogRevision, definition, identity, scheduledRetirement }) =>
          Effect.succeed(
            scheduledRetirement === undefined
              ? { catalogRevision, definition, identity }
              : { catalogRevision, definition, identity, scheduledRetirement },
          ),
        currentness_failure: ({ candidateDefinitionRevisionIds, reason }) =>
          Effect.fail(
            new PriceGroupCurrentnessFailure({
  ...
  const readDefinitionRevision: PriceGroupCatalogPersistence['readDefinitionRevision'] = Effect.fn(
    ...
    return yield* Match.value(outcome).pipe(
      Match.tags({
        ...
        found: ({ definition, identity, scheduledRetirement }) =>
          Effect.succeed(
            scheduledRetirement === undefined
              ? { definition, identity }
              : { definition, identity, scheduledRetirement },
          ),
        not_found: () => Effect.fail(notFound()),
      }),
      Match.exhaustive,
