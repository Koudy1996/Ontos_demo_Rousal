...
describe('Price Group identity, definition, and lifecycle contracts', () => {
  ...
  it('accepts immutable revisions only with a valid predecessor chain and exact contract support', () => {
    ...
    const successor = {
      ...definition,
      classificationPurpose: 'Clarified dealer eligibility without changing classification meaning.',
    ...
    expect(decode(successor)).toMatchObject({
      classificationPurpose: successor.classificationPurpose,
      semanticContinuity: { comparedDefinitionRevisionId: definitionRevisionId, decision: 'SAME_MEANING' },
    });
    expect(() =>
      decode({
        ...successor,
        semanticContinuity: {
          ...successor.semanticContinuity,
          comparedDefinitionRevisionId: successor.definitionRevisionId,
        },
      }),
    ).toThrow();
