...
const payload = Schema.decodeUnknownSync(CreatePriceGroupDefinitionRevisionPayloadSchema)({
  ...
  semanticDecision: {
    comparedDefinitionRevisionId: previousDefinitionRevisionId,
...
const definition = Schema.decodeUnknownSync(PriceGroupDefinitionRevisionSchema)({
  ...
  meaningFingerprint: payload.expectedCurrent.meaningFingerprint,
...
it.effect('persists expected-current evidence and emits the revision event with its attached outbox message', () =>
  Effect.gen(function* createsRevision() {
    ...
    expect(persistedInput).toMatchObject({
      ...
      reason: payload.reason,
      semanticDecision: payload.semanticDecision,
...
it('requires SAME_MEANING to name the exact expected predecessor and rejects caller-owned fingerprints', () => {
  expect(() =>
    Schema.decodeUnknownSync(CreatePriceGroupDefinitionRevisionPayloadSchema)({
      ...payload,
      semanticDecision: {
  ...
  expect(() =>
    Schema.decodeUnknownSync(CreatePriceGroupDefinitionRevisionPayloadSchema, { onExcessProperty: 'error' })({
      ...payload,
      meaningFingerprint: fingerprint,
    }),
  ).toThrow();
