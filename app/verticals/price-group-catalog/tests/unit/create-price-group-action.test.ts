...
import { CreatePriceGroupPayloadSchema } from '../../shared/actions/create-price-group.ts';
...
const fingerprint = 'b20b0775b826310b625c71045cdb6d88401be2c86f8634373e6c3124a97b12fd';
...
const businessResult = {
  ...
  identity: {
    ...
    lifecycle: { activeFrom: definition.effectivePeriod.effectiveFrom, retiredAt: null, state: 'ACTIVE' as const },
    meaningFingerprint: definition.meaningFingerprint,
    priceGroupRef: definition.priceGroupRef,
  },
...
describe('Create Price Group Action', () => {
  it('keeps semantic identity owner-issued and rejects a known future definition gap', () => {
    const decode = Schema.decodeUnknownSync(CreatePriceGroupPayloadSchema, { onExcessProperty: 'error' });

    expect(() => decode({ ...payload, meaningFingerprint: 'a'.repeat(64) })).toThrow();
    expect(() => decode({ ...payload, effectiveTo: '2026-12-01T00:00:00.000Z' })).toThrow();
  });
  ...
  it.effect('persists the exact request and records audit, data-access, event, and linked outbox evidence', () =>
    Effect.gen(function* createAndCollectEvidence() {
      ...
      expect(result).toEqual(businessResult);
    ...
    expect(
      mapCreatePriceGroupActionProblem(
        new PriceGroupSemanticIdentityConflict({
          ...
          existingMeaningFingerprint: fingerprint,
          existingPriceGroupRef: definition.priceGroupRef,
          reason: 'The semantic identity belongs to another Price Group.',
          requestedMeaningFingerprint: fingerprint,
        }),
      ),
