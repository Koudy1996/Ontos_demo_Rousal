...
it('owns global gateway assertion replay protection outside tenant RLS', () => {
  const redemptions = getTableConfig(gatewayAssertionRedemptions);

  expect(redemptions.enableRLS).toBe(false);
  expect(redemptions.policies).toEqual([]);
  expect(redemptions.columns.map((column) => [column.name, column.notNull])).toEqual([
    ['audience', true],
    ['expires_at', true],
    ['issuer', true],
    ['jti', true],
  ...
  expect(redemptions.uniqueConstraints.map((constraint) => constraint.name)).toEqual([
    'price_group_catalog_gateway_assertion_redemptions_identity_uk',
  ]);
  expect(redemptions.indexes.map((definitionIndex) => definitionIndex.config.name)).toEqual([
    'price_group_catalog_gateway_assertion_redemptions_expiry_idx',
  ]);
});
