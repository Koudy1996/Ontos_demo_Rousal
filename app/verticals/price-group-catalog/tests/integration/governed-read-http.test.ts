...
import { makePriceGroupCatalogApiRuntime } from '../../api/index.ts';
...
const definition = Schema.decodeUnknownSync(PriceGroupDefinitionRevisionSchema)({
  ...
  revisionNumber: 3,
  semanticContinuity: {
    comparedDefinitionRevisionId: '88888888-8888-4888-8888-888888888888',
...
const unusedActionRuntime: ActionRuntimeService = {
  resolveActionCommit: () => Effect.die('Action runtime is outside this governed Read proof'),
  runAction: () => Effect.die('Action runtime is outside this governed Read proof'),
};
...
describe('Price Group governed Read HTTP integration', () => {
  ...
  it.effect('accepts an assertion once, then rejects its replay before executing another Read', () =>
    Effect.gen(function* rejectAssertionReplay() {
      const assertion = yield* makeAssertion();
      const harness = makeReadRuntime();
            ...
            if (consumed.has(key)) {
              return Effect.fail(
                new GatewayAssertionReplayError({
                  reason: 'private replay-storage conflict detail',
                }),
              );
            }
            consumed.add(key);
      ...
      const accepted = yield* Effect.promise(() => handle(runtime, requestFor(endpoint, assertion.token)));
      expect(accepted.status).toBe(200);
      expect(harness.readCount()).toBe(1);

      const replayed = yield* Effect.promise(() => handle(runtime, requestFor(endpoint, assertion.token)));
      expect(replayed.status).toBe(401);
      expect(replayed.headers.get('www-authenticate')).toBe('Bearer');
      const problem = yield* readProblem(replayed);
      expect(Schema.is(endpoint.authenticationProblem)(problem)).toBe(true);
      expect(problem.status).toBe(401);
      expectSanitized(problem, ['private replay-storage conflict detail', assertion.token, principalId, tenantId]);
      expect(harness.readCount()).toBe(1);
    }),
  );
  ...
  it.live('redeems once through the production runtime constructor and real owner database layer', () =>
    Effect.gen(function* redeemThroughProductionComposition() {
      ...
      const accepted = yield* Effect.promise(() => runtime.handler(requestFor(endpoint, assertion.token)));
      expect(accepted.status).toBe(200);
      expect(harness.readCount()).toBe(1);

      const replayed = yield* Effect.promise(() => runtime.handler(requestFor(endpoint, assertion.token)));
      expect(replayed.status).toBe(401);
      expect(replayed.headers.get('www-authenticate')).toBe('Bearer');
      const problem = yield* readProblem(replayed);
      expect(Schema.is(endpoint.authenticationProblem)(problem)).toBe(true);
  ...
  it.effect('rejects an otherwise valid assertion for any non-exact audience before redemption or Read execution', () =>
    Effect.gen(function* rejectWrongAudience() {
