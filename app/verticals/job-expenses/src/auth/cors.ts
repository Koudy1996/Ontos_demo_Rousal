...
export const jobExpensesCors = (shellOrigin: string) =>
  HttpRouter.cors({
    allowedHeaders: [
      'Accept',
      'Accept-Language',
...
export const JobExpensesCorsLive = Layer.unwrap(
  Config.string('ONTOS_GATEWAY_ISSUER').pipe(Effect.map(jobExpensesCors)),
);
