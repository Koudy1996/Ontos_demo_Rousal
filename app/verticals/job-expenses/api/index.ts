...
import { jobExpensesApi, jobExpensesOperationContexts } from '../shared/api.ts';
import type { OperationContext } from '../shared/api.ts';
import { ultramodernApiMarker } from '../shared/ultramodern-build.ts';
import { JobExpensesCorsLive } from '../src/auth/cors.ts';
...
import { jobEconomicsReadApiLive } from './job-economics-read-server.ts';
import { jobExpenseListReadApiLive } from './job-expense-list-read-server.ts';
import { jobExpensesCommitStatusReadApiLive } from './job-expenses-commit-status-read-server.ts';
import { jobSelectionReadApiLive } from './job-selection-read-server.ts';
import { recordJobExpenseActionApiLive } from './record-job-expense-action-server.ts';
import { updateJobExpenseActionApiLive } from './update-job-expense-action-server.ts';
import { voidJobExpenseActionApiLive } from './void-job-expense-action-server.ts';
// </generated-governed-http-handler-imports>
...
export const governedReadApiHandlersLive = Layer.mergeAll(
  ...
  jobEconomicsReadApiLive.pipe(GovernedReadLayer.provide(governedReadRuntimeLive)),
  jobExpenseListReadApiLive.pipe(GovernedReadLayer.provide(governedReadRuntimeLive)),
  jobExpensesCommitStatusReadApiLive.pipe(
    GovernedReadLayer.provide(governedReadRuntimeLive),
    Layer.provide(governedActionRuntimeLive),
  ),
  jobSelectionReadApiLive.pipe(GovernedReadLayer.provide(governedReadRuntimeLive)),
  recordJobExpenseActionApiLive.pipe(GovernedReadLayer.provide(governedActionRuntimeLive)),
  updateJobExpenseActionApiLive.pipe(GovernedReadLayer.provide(governedActionRuntimeLive)),
  voidJobExpenseActionApiLive.pipe(GovernedReadLayer.provide(governedActionRuntimeLive)),
  // </generated-governed-http-handler-layers>
).pipe(
...
const jobExpensesReadinessLayer = HttpApiBuilder.group(jobExpensesApi, 'foundation', (handlers) =>
  handlers.handle('readiness', () =>
    Effect.succeed({
      ...
      Effect.withSpan('ultramodern.api.jobExpenses.readiness', {
        attributes: operationAttributes(jobExpensesOperationContexts.readiness),
        kind: 'server',
      }),
    ),
  ),
...
const apiHandlersLive = Layer.mergeAll(jobExpensesReadinessLayer, governedReadApiHandlersLive).pipe(
  ...
  GovernedReadLayer.provide(JobExpensesCorsLive),
  GovernedReadLayer.provide(dependencyReadTransportLive),
  GovernedReadLayer.orDie,
);
export const makeJobExpensesApiRuntime = () =>
  assembleEffectBffRuntime({
    api: jobExpensesApi,
    handlers: apiHandlersLive,
  });
const apiRuntime = makeJobExpensesApiRuntime();
