  ...
  suite.effect('registers Party ownership in application database grants and exact verification', () =>
    Effect.gen(function* testScenario() {
      ...
      expect(verifier).toMatch(
        /const EXPECTED_APPLICATION_SCHEMAS = \[\s*'auth',\s*'catalog',\s*'commerce_customer_context',\s*'commerce_market_catalog',\s*'contacts',\s*'core',\s*'party',\s*'payment_term_catalog',\s*\]/u,
      );
