-- Canonical owner evidence is bound separately from the closed assignment routines so their
-- cardinality, overlap, replay, resolution, and removal behavior remain byte-for-byte intact.
CREATE FUNCTION "commerce_customer_context"."read_price_group_assignment_compatibility_evidence"(
  p_tenant_id uuid,
...
BEGIN
  ...
  THEN
    RETURN QUERY SELECT 'NOT_FOUND'::text, NULL::integer, NULL::text, NULL::integer,
  ...
  v_outcome := CASE
    WHEN v_assignment.definition_revision_id IS NULL
    ...
    THEN 'LEGACY'
    WHEN v_assignment.definition_revision_id IS NOT NULL
      AND v_assignment.meaning_fingerprint IS NOT NULL
      AND v_assignment.definition_effective_from IS NOT NULL
      AND v_assignment.compatibility_trusted_at IS NOT NULL
      AND v_assignment.compatibility_verified_at IS NOT NULL
    THEN 'FOUND'
    ELSE 'CORRUPT'
  END;

  RETURN QUERY SELECT v_outcome, v_assignment.catalog_revision,
...
CREATE FUNCTION "commerce_customer_context"."bind_price_group_assignment_compatibility_evidence"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_assignment_id text,
...
$$;
--> statement-breakpoint
REVOKE ALL ON FUNCTION "commerce_customer_context"."read_price_group_assignment_compatibility_evidence"(uuid, uuid, text) FROM PUBLIC;
--> statement-breakpoint
GRANT EXECUTE ON FUNCTION "commerce_customer_context"."read_price_group_assignment_compatibility_evidence"(uuid, uuid, text) TO "ontos_runtime";
--> statement-breakpoint
