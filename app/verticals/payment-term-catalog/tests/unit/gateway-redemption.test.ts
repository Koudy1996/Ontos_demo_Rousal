...
import { PaymentTermCatalogDatabase } from '../../src/database/client.ts';
import { paymentTermCatalogRelations } from '../../src/database/schema.ts';
...
const fixture = (execute: (sql: string, params: readonly unknown[]) => Effect.Effect<readonly object[], SqlError>) =>
  Effect.gen(function* wireFixture() {
    ...
    const executor = yield* makeWithDefaults({ relations: paymentTermCatalogRelations }).pipe(
      Effect.provideService(PgClient.PgClient, client),
    );
    const services = yield* Layer.build(
      GatewayAssertionRedemptionLive.pipe(Layer.provide(Layer.succeed(PaymentTermCatalogDatabase, { executor }))),
    );
