...
const bffPrefix = '/operations-dashboard-api';
const cloudflareWorkerName = 'app-operations-dashboard';
const build = createModernBuildContext({
  appId,
  cloudflarePublicUrlEnvironmentVariable: 'ULTRAMODERN_PUBLIC_URL_OPERATIONS_DASHBOARD',
  cloudflareWorkerName,
  defaultPort: 4114,
  getBuildConfigEnvironment,
  portEnvironmentVariable: 'VERTICAL_OPERATIONS_DASHBOARD_PORT',
});
const resolveApiBaseUrl = () => {
  const publicUrl = build.envValue('ULTRAMODERN_PUBLIC_URL_OPERATIONS_DASHBOARD');
  if (publicUrl !== undefined) {
    return new URL(bffPrefix, publicUrl).href;
  }
  if (build.cloudflareDeployEnabled) {
    throw new Error('OperationsDashboard deployment requires ULTRAMODERN_PUBLIC_URL_OPERATIONS_DASHBOARD.');
  }
...
const config = createModernConfig({
  ...
  bffPrefix,
  build,
  chunkLoadingGlobal: '__ULTRAMODERN_VERTICAL_OPERATIONS_DASHBOARD_LOADED_CHUNKS__',
  cloudflareWorkerName,
  ...
  uniqueName: 'verticalOperationsDashboard',
});
  ...
  presetUltramodern(
    {
      ...
      source: {
        ...
        globalVars: {
          ...
          ULTRAMODERN_OPERATIONS_DASHBOARD_API_BASE_URL: resolveApiBaseUrl(),
        },
      },
    },
    {
      appId,
      deliveryUnit: { buildMarker: '3241c619985c4ed9', unitId: 'app/operations-dashboard', version: '0.1.0' },
    },
  ),
);
