...
A derived, rebuildable, context-bounded read model for Listing/Search. It is never canonical Assortment authority: a hit is not authoritative `ELIGIBLE`, omission is not authoritative `INELIGIBLE`, and projection completeness is not Owner-Verifiable Set Completeness Evidence. Product inclusion is allowed only while the projection contract establishes disclosure-safe inclusion for the exact Guest/Purchasing Subject and commercial context. Stale, uncertain, unverifiable, or wrong-scope entries are omitted rather than optimistically exposed. Known invalidation ends use of the old positive entry until a replacement disclosure-safe state exists. Event silence, TTL, cache age, index completeness, projection revision/hash, or pagination do not by themselves prove continued disclosure safety. A broader shared/Guest projection cannot replace an uncertain narrower subject-sensitive slice without explicitly proven disclosure equivalence.
_Avoid_: Search/index as System of Record, stale positive hit retained because detail will recheck later, omission treated as DENY, cross-subject or cross-scope cache reuse without proven equivalence, shared fallback used to broaden an uncertain personalized result.

**Partial/Degraded Assortment Discovery Result**:
A Listing/Search result that returns only the Products whose projection inclusion remains disclosure-safe while one or more other candidate Products or slices are omitted because their disclosure state is stale, uncertain, unverifiable, or wrong-scope. It is a discovery/result-set state, not an Assortment Decision Outcome, and does not claim that the returned set is the complete canonical set of visible Products. If every candidate is uncertain, an empty partial/degraded result is valid without implying that every Product is `VISIBILITY=INELIGIBLE`.
_Avoid_: Partial result interpreted as canonical completeness, omitted Product treated as INELIGIBLE, whole-request failure required for one local uncertainty, uncertain Product retained to improve recall.

**Assortment Invalidation Event**:
A notification that a committed Assortment Rule/Binding/Closed Boundary change may require derived consumers to refresh or rebuild. Receipt is not a decision, not Owner-Verifiable Set Completeness Evidence, and absence or delay is not Current proof. For Listing/Search, a known invalidation may remove affected projection inclusion until replacement disclosure-safe state exists, but the event does not determine the new canonical VISIBILITY outcome. It also does not revoke an already-issued unexpired Assortment Commitment Confirmation.
...
The Launch condition that every Launch-critical legacy behavior has sufficient evidence, an explicit migration disposition, canonical identities and ownership, a supported canonical representation, the required Currentness/Set Completeness contract for affected authoritative decisions, the exact-context Listing/Search disclosure-safety contract for derived discovery, Set/component PURCHASE composition where applicable, and bounded Confirmation semantics through commitment. Ordinary retained behavior uses Rule/Binding. Proven true closed per-subject behavior may `TRANSFORM` to one immutable complete Closed Assortment Boundary through standard Boundary Actions/Permissions. Unproven identity, closure meaning, Admission Set completeness, scope, lifecycle, conflicting Boundary composition, required material set completeness, unsafe projection disclosure, or unresolved commitment proof semantics blocks the affected cutover journey.
_Avoid_: Raw record parity, guessed canonical state, unresolved-but-enabled behavior, `SUBJECT ALL=DENY + ALLOW` claimed as true closed-whitelist equivalence, partial rows imported as complete Boundary, stale/uncertain Search hit retained for recall, partial/degraded Search result presented as canonical complete visibility.
