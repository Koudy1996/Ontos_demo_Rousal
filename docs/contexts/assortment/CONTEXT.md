...
A Listing/Search result that knowingly omits one or more Products or slices within the requested discovery scope because the Assortment Discovery Disclosure Contract cannot establish safe inclusion for them, while returning the remaining Products whose inclusion is covered. It is a discovery/result-set state, not an Assortment Decision Outcome. It signals known omission only: it does not claim canonical visibility completeness, and the absence of this state never upgrades a Search projection to Owner-Verifiable Set Completeness Evidence or a complete canonical visible-Product set. If every Product in the requested discovery scope lacks safe inclusion coverage, an empty Partial Assortment Discovery Result is valid without implying that every Product is `VISIBILITY=INELIGIBLE`.
_Avoid_: Generic degraded state, Partial interpreted as canonical completeness, absence of Partial interpreted as canonical completeness, omitted Product treated as INELIGIBLE, whole-request failure required for one local uncertainty, uncertain Product retained to improve recall.
