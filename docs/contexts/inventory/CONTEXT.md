...
**Stock Sharing Eligibility** — Inventory-owned positive Current relation between one exact Stock Position and a qualified commercial scope: required Selling Legal Entity + Channel, optionally restricted by Commerce Market and/or Storefront. All specified dimensions must match the trusted Current Commerce Purchasing Context; an omitted optional dimension imposes no restriction on that dimension and does not bypass Commerce context validation. At least one proven applicable Current relation establishes sharing eligibility; proven absence or unverifiable evidence does not fabricate a positive result. There is no DENY or specificity precedence: a narrower relation does not override a broader one. Establishing, changing, or ending relations affects new Reservations only, preserving existing Reservations, original commercial scope and historical Allocations; ending one relation does not remove access while another applicable Current relation remains. Tenant/Customer Configuration isolation, Principal Permission, Assortment and Reservation guarantees remain independent boundaries.
_Avoid_: a new durable Sales Context Resource, the entire runtime Commerce Purchasing Context as stored subject, Purchasing Subject/Commerce Customer Group/Retail Customer/Counterparty/Price Group selectors, shared Channel/seller/Product/Storefront/address as implicit shared-capacity authority, eligibility as Permission or a second Assortment resolver.
