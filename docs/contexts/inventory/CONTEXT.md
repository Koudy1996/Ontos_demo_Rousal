...
**Stock Item** — Durable Inventory Resource representing exactly one exact Catalog Selection meaning with one explicit stock Unit. A materially different exact Selection meaning uses a different Stock Item.
_Avoid_: Product, SKU, or one Stock Item shared by materially different exact Catalog Selections.
...
**Catalog-to-Stock Binding** — Inventory-owned Current one-to-one relation between one exact Catalog Selection meaning and one exact Stock Item. Historical use remains explainable when the Current relation later changes or ends.
_Avoid_: inference from Product, SKU, Package contents, Set components, source identifiers, or availability.
...
**External Stock Correlation** — Inventory-owned Current relation from one issuer/namespace/kind-qualified external identifier to exactly one corresponding Stock Item or Stock Location. The external identifier remains correlation evidence, not canonical Inventory identity.
_Avoid_: similarity-based SKU/name/address matching or an external identifier treated as Inventory identity.
...
**Provisional Shortage Priority** — Launch priority for competing unprotected provisional Reservation Confirmations after a material shortage: oldest owner-issued still-valid Confirmation first within the affected constrained scope.
_Avoid_: physical picking order, best-fit, customer/channel priority, or technical arrival order.
...
**Inventory Source Assertion** — Provenance-backed stock claim from the selected External Business System, retaining issuer, exact correlated scope, fact meaning, Quantity + Unit, business time, and owner-defined ordering evidence. Transport arrival does not make an assertion Current.
